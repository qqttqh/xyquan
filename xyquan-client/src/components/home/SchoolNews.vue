<template>
    <div class="root" v-infinite-scroll="load" infinite-scroll-distance="200">
...
import { getSchoolNewsPosts } from '@/api/getPosts'
...
const count = ref(1)
function load() {
    getSchoolNewsPosts(count.value)
    count.value += 1
}
// 过滤出日常贴
const posts = computed(() => {
    return postStore.data.filter(post => post.postGroup === 'SchoolNews')
})
</script>
